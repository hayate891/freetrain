...
//				return true;
		...
		private Color mapColor( Color c ) {
			if( World.world.clock.dayOrNight==DayNight.DayTime ) return c;
			else	return ColorMap.getNightColor(c);
		}
		...
		public void setTrees(Sprite ground, Sprite[] trees, byte[] indices, int price) 
		{
			...
			treePrice = price;
			if( voxelAbove != null )
				World.world.remove(voxelAbove.location);
			Location l = location;
			l.z++;
			voxelAbove = new VoxelImpl( this, l);
		}
		...
		static MountainVoxel() {
			...
			for( int tdiff=0; tdiff<=8; tdiff++ ) {// tdiff= difference between top and bottom
...
//						new Rectangle( new Point(0,0), images[tdiff].size )); // clear the surface
		...
		/// </summary>
		[Serializable]
		protected internal class VoxelImpl : AbstractVoxelImpl {
			protected internal VoxelImpl( MountainVoxel _owner, Location _loc )
				: base(_loc) {
				owner = _owner;
			}

			protected MountainVoxel owner;
			

			public override void draw( DrawContext display, Point pt, int heightCutDiff  ) {
				owner.draw(display,new Point(pt.X,pt.Y+16),heightCutDiff);
			}
