...
namespace nft.util
{
	...
	public class Properties : Property, IEnumerable
	{
		...
		static public Properties LoadFromFile(string filename, bool Readonly)
		{
			...
			try
			{
				// ここでバンドルできない例外が発生したら、おそらくプロパティファイルの記述ミス
				root = doc.SelectSingleNode("properties");
			...
			finally
			{
				if(root==null)
				{
					string text = string.Format("PropertyFileError:{0}",filename);
					UIUtil.ShowErrorMessageBox(null,text,null);
				}
			}
		}
